---
name: server-admin
description: 服务器管理（Squid, CentOS 8, SSH 2002）
trigger: 用户询问服务器状态、管理用户和权限、查看系统资源、SSH连接问题、Docker部署或任何服务器维护任务
---
...
- **SSH 别名**: `Squid`
- **SSH 端口**: 2002
- **SSH 密钥**: `~/.ssh/WorkComputer`（已配置免密登录）
- **用户**: root（密钥+密码 `Zhy26825.+8`）、OnePiece（uid=1000, wheel组, sudo NOPASSWD）
...
- PQ KEX 警告：OpenSSH 版本较旧，提示 "store now, decrypt later" 风险，不影响正常使用
...
参考 `docs/docker-compose/server-setup.md` 中的完整安装流程。
...
**注意事项：** 密码含特殊字符（如 `+`、`.`）时需适当转义；生产环境建议使用外部 Vault 管理密码。
